...
class CameraNode:
    ...
    def inversePerspective(self,image):

        srcPoint = np.float32([[192, 195], [135, 361], [349, 354], [369, 198]])
        dstPoint = np.float32([[0, 0], [0, 300], [400, 300], [400, 0]])
        ...
        while not rospy.is_shutdown():                  # ROS가 종료되지 않은 동안
            ...
            if ret:                                     # 이미지가 정상적으로 읽혀진 경우
                try:
                    ...
                    frame = self.inversePerspective(frame)
